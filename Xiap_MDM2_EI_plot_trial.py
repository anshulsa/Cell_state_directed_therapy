# -*- coding: utf-8 -*-
"""
Created on Sun Jun  5 01:29:03 2022
...
def TMZ_mdm2_xiap_epigen_ode(s, t, epi_state, tmz_fun, sw):
...
def mdm2_xiap_plot_fun(tmz_val, mdm2_multiplier, xiap_multiplier):
    ...
    trans_state = [0, 1, 1]   # [MGMT, MDM2, X]
    epi_state =   [zero_fun, mdm2_multiplier,  xiap_multiplier]   # 
    
    y = spi.odeint(TMZ_mdm2_xiap_epigen_ode, y0, t, args=(epi_state, tmz_fun, trans_state,));
...
resgen_mat_xiap = np.full((100), 1)
pergen_mat_xiap  = np.full((100), 1.5)
epigen_mat_xiap = [i*1.5 for i in epigenetic_module(150)] 

resgen_mat_mdm2 = np.full((100), 1)
pergen_mat_mdm2  = np.full((100), 20)
epigen_mat_mdm2 = [i*20 for i in epigenetic_module(150)]  

time_array = np.linspace(0, 150, 100)

epigen_fun_xiap  = InterpolatedUnivariateSpline(time_array, epigen_mat_xiap, k = 1)   
resgen_fun_xiap  = InterpolatedUnivariateSpline(time_array, resgen_mat_xiap, k = 1) 
pergen_fun_xiap  = InterpolatedUnivariateSpline(time_array, pergen_mat_xiap, k = 1)  
epigen_fun_mdm2  = InterpolatedUnivariateSpline(time_array, epigen_mat_mdm2, k = 1)   
resgen_fun_mdm2  = InterpolatedUnivariateSpline(time_array, resgen_mat_mdm2, k = 1) 
pergen_fun_mdm2  = InterpolatedUnivariateSpline(time_array, pergen_mat_mdm2, k = 1)  
 
model_output_array = []

test1 = mdm2_xiap_plot_fun(334.6, resgen_fun_mdm2, resgen_fun_xiap)
model_output_array.append(test1)
test2 = mdm2_xiap_plot_fun(334.6, pergen_fun_mdm2, pergen_fun_xiap)
model_output_array.append(test2)
test3 = mdm2_xiap_plot_fun(334.6, epigen_fun_mdm2, epigen_fun_xiap)
model_output_array.append(test3)
...
axs[0, 0].set_title('MDM2 and XIAP (Restrictive)')
axs[0, 1].plot(model_output_array[0][0], model_output_array[1][1])
axs[0, 1].set_title('MDM2 and XIAP  (Permissive)')
axs[0, 2].plot(model_output_array[0][0], model_output_array[2][1])
axs[0, 2].set_title('MDM2 and XIAP with EI')
...
axs[2, 0].plot(model_output_array[0][0], model_output_array[0][3])
axs[2, 0].set_ylabel('MDM2 (nm)')
axs[2, 1].plot(model_output_array[0][0], model_output_array[1][3])
axs[2, 2].plot(model_output_array[0][0], model_output_array[2][3])

axs[3, 0].plot(model_output_array[0][0], model_output_array[0][4])
axs[3, 0].set_ylabel('XIAP (nm)')
axs[3, 1].plot(model_output_array[0][0], model_output_array[1][4])
axs[3, 2].plot(model_output_array[0][0], model_output_array[2][4])

axs[4, 0].plot(model_output_array[0][0], model_output_array[0][5])
axs[4, 0].set_ylabel('P53 active (nm)')
axs[4, 1].plot(model_output_array[0][0], model_output_array[1][5])
axs[4, 2].plot(model_output_array[0][0], model_output_array[2][5])

axs[5, 0].plot(model_output_array[0][0], model_output_array[0][6])
axs[5, 0].set_ylim(0, 700)
axs[5, 0].set_ylabel('cPARP (nm)')
axs[5, 0].set_xlabel('Time (Hrs)')

axs[5, 1].plot(model_output_array[0][0], model_output_array[1][6])
axs[5, 1].set_ylim(0, 700)
axs[5, 1].set_xlabel('Time (Hrs)')

axs[5, 2].plot(model_output_array[0][0], model_output_array[2][6])
axs[5, 2].set_ylim(0, 700)
axs[5, 2].set_xlabel('Time (Hrs)')
